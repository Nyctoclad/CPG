...
//					}
			...
			Color StarDomeColor = star_dome_Material.GetVector("_TintColor");
...
//				bool is_dayToDusk = (AutoSunPosition && Rot_Sun_X > 15 && Rot_Sun_X < 45) | (!AutoSunPosition && Current_Time <  (16.1f + Shift_dawn) & Current_Time > (11f + Shift_dawn));
		...
		//	if(Current_Time < 16 & Current_Time > 9 ){
				if (is_DayLight) {
					...
					if (AutoSunPosition && is_after_17) {//if(Current_Time > 17){ //BREAK stars near morning //v3.3
...
//				}else{
				...
				} else if (is_after_22) { //if(Current_Time <=9 & Current_Time >=0){ //v3.3
